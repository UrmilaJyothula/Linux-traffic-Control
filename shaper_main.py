...
db = MySQLdb.connect(host="localhost", user="root",passwd="sreekv333",db="lts")
cursor = db.cursor()
...
def ifaces():
	if (request.args.get('uname') == "lts" and request.args.get('pass') == "atslabb00"):
	...
	if (request.args.get('uname') == "lts" and request.args.get('pass') == "atslabb00"):
		if(request.args.get('iface')):
			...
			if (request.args.get('action') == "add"):
				...
				sql_check = 'select * from `'+ip+'`'
				cursor.execute(sql_check)
				table_check = cursor.fetchall()
				if (table_check) :
					e = "table exists"
				else: 
					sql = 'create table "'+ip+'" (id int AUTO_INCREMENT PRIMARY KEY, delay varchar(15) DEFAULT "0ms", jitter varchar(15) DEFAULT "0ms", loss varchar(15) DEFAULT "0%", interface varchar(15))'
					cursor.execute(sql)
					db.commit()
				...
				line3 = "Traffic Shaped on interface: "+interface+"\n<br>"
				line4 = "Delay: "+delay+"\n<br>jitter:"+jitter+"\n<br>loss: "+loss+"\n<br>"
				line5 = "-------------------" 
